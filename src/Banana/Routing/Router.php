...
class Router
{
    ...
    public function __construct(string $format, string $lang)
    {
        $this->format = $format;
        $this->lang = $lang;
    }
    ...
    public function buildUrl(string $method, string $routeName, array $parameters = []): string
    {
        ...
        $path = preg_replace_callback(Route::PARAM_PATTERN, function ($matches) use ($parameters) {
            $paramName = $matches[1];
        ...
        return sprintf($this->format, $this->lang, $path);
    }
